...
public class HistoryView {
    ...
    private static final String TITLE = "Lịch sử thanh toán";
    ...
    public static JPanel createPaymentHistoryPanel(String username) {
        ...
        HistoryController controller = new HistoryController(username);

        JPanel buttonPanel = new JPanel();
        buttonPanel.setLayout(new BoxLayout(buttonPanel, BoxLayout.Y_AXIS));
        buttonPanel.setBorder(new TitledBorder(TITLE));
        buttonPanel.setBackground(Color.WHITE);

        Object[][] data = controller.getPaymentData();
        for (int i = 0; i < data.length; i++) {
            final int index = i; // Ensure effectively final for lambda
            Object[] row = data[index];
            if (controller.isValidPayment(row)) {
                String bookingId = row[1].toString();
                String notificationMessage = controller.formatPaymentMessage(row);
                boolean isRead = controller.isBookingRead(bookingId);
                JButton button = createStyledButton(notificationMessage, bookingId, isRead, controller);
                buttonPanel.add(button);
    ...
    private static JButton createStyledButton(String text, String bookingId, boolean isRead, HistoryController controller) {
        String htmlText = "<html>" + text + (isRead ? " [Đã đọc]" : "") + "</html>";
        JButton button = new JButton(htmlText);
        button.setFocusPainted(false);
        button.setCursor(new Cursor(Cursor.HAND_CURSOR));
        ...
        button.addActionListener(e -> controller.showNotificationDialog(button, bookingId));
