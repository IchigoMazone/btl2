...
public class CustomerView {
    ...
    private static DefaultTableModel tableModel;
    private static JTable table;
    private static JTextField tfTuKhoa;
    private static JComboBox<String> cbTieuChi;

    public static void setBookingService(BookingService service) {
        CustomerController.setBookingService(service);
    }
    ...
    public static JPanel createCustomerSearchPanel() {
        ...
        cbTieuChi = new JComboBox<>(new String[]{
                "Tài khoản", "Số điện thoại", "Gmail", "Họ tên", "Mã định danh", "Mã phòng"
        });
        tfTuKhoa = new JTextField(20);
        ...
        controller.loadPersonData(tableModel);
        ...
        // Button actions
        btnTimKiem.addActionListener(e -> controller.handleSearch(tfTuKhoa.getText().trim(), (String) cbTieuChi.getSelectedItem(), tableModel, panel));
        btnThem.addActionListener(e -> controller.showCustomerDialog(panel, tableModel, null));
        btnXoa.addActionListener(e -> controller.handleDelete(table.getSelectedRow(), tableModel, panel));
        btnSua.addActionListener(e -> controller.handleEdit(table.getSelectedRow(), tableModel, panel));
    ...
    public static void showCustomerDialog(JPanel parent, DefaultTableModel model, AbstractMap.SimpleEntry<Integer, AbstractMap.SimpleEntry<String, org.example.entity.Person>> editData, CustomerController controller, java.util.List<String> bookingIds) {
        JTextField tfHoTen = new JTextField(22);
        JTextField tfDinhDanh = new JTextField(22);
        JComboBox<String> cbDocType = new JComboBox<>(new String[]{"CCCD", "Hộ chiếu", "Mã định danh"});
        ...
        String bookingId = null;
        if (editData != null) {
            org.example.entity.Person person = editData.getValue().getValue();
            bookingId = editData.getValue().getKey();
            tfHoTen.setText(person.getFullName());
            tfDinhDanh.setText(person.getDocumentCode());
            cbDocType.setSelectedItem(person.getDocumentType() != null ? person.getDocumentType() : "CCCD");
        ...
        JComboBox<String> cbBookingId = null;
        if (editData == null && bookingIds != null && !bookingIds.isEmpty()) {
            cbBookingId = new JComboBox<>(bookingIds.toArray(new String[0]));
            gbc.gridy++; gbc.gridx = 0;
            inputPanel.add(new JLabel("Mã booking:"), gbc); gbc.gridx = 1;
            inputPanel.add(cbBookingId, gbc);
        ...
        if (result == JOptionPane.OK_OPTION) {
            controller.handleDialogSubmit(tfHoTen.getText().trim(), tfDinhDanh.getText().trim(), (String) cbDocType.getSelectedItem(),
                    editData, cbBookingId != null ? (String) cbBookingId.getSelectedItem() : bookingId, model, parent);
        }
    }
    ...
    public static JTextField getTfTuKhoa() {
        return tfTuKhoa;
    }

    public static JComboBox<String> getCbTieuChi() {
        return cbTieuChi;
    }
}