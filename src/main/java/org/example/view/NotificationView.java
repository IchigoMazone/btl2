...
public class NotificationView {
    ...
    public static JPanel createUserNotificationPanel(String username) {
        ...
        NotificationController controller = new NotificationController(username);
        ...
        model = new DefaultTableModel(TABLE_COLUMNS, 0) {
            @Override
            public boolean isCellEditable(int row, int col) {
                return true; // Allow editing to trigger editor
            }
        };
        ...
        table.setDefaultRenderer(Object.class, new ButtonLikeRenderer());
        table.getColumnModel().getColumn(0).setCellEditor(
                new ButtonLikeEditor(new JCheckBox(), controller)
        );
    ...
    static class ButtonLikeEditor extends DefaultCellEditor {
        ...
        public ButtonLikeEditor(JCheckBox checkBox, NotificationController controller) {
            super(checkBox);
            ...
            button.addActionListener(e -> {
                fireEditingStopped();
                controller.showNotificationDialog(selectedRow, model);
            });
        }
