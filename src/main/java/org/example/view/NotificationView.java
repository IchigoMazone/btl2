...
import org.example.action.CheckNotification;
...
public class NotificationView {
    ...
    public static JPanel createUserNotificationPanel(String username) {
        ...
        List<Notification> filtered = controller.getUserNotifications(username);
        ...
        CheckNotification.loadTableData(filtered, model);

        table.setDefaultRenderer(Object.class, new CheckNotification.ButtonLikeRenderer());
        table.getColumnModel().getColumn(0).setCellEditor(
                new CheckNotification.ButtonLikeEditor(new JCheckBox(), model, filtered, controller)
        );
