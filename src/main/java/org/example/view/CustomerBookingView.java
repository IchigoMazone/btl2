...
public class CustomerBookingView {
    ...
    public static JPanel createBookingPanel(MainFrameView mainFrame, String username, SelectedRoomInfo selectedRoom) {
        ...
        CustomerBookingController controller = new CustomerBookingController(mainFrame, username, selectedRoom);

        JPanel formPanel = createFormPanel(controller);
        JPanel buttonPanel = createButtonPanel(mainFrame, controller);

        panel.add(formPanel);
        panel.add(Box.createVerticalStrut(10));
        panel.add(buttonPanel);

        cbSoNguoi.addActionListener(e -> controller.handleSoNguoiChange());

        return panel;
    }

    private static JPanel createFormPanel(CustomerBookingController controller) {
        JPanel formPanel = new JPanel(new GridBagLayout());
        formPanel.setBackground(UIManager.getColor("Panel.background"));
        formPanel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createLineBorder(new Color(180, 180, 180)),
        ...
        cbLoaiChung.addActionListener(e -> controller.handleLoaiChungChange());
        formPanel.add(cbLoaiChung, gbc);
        gbc.gridx = 2;
        formPanel.add(new JLabel("Mã:"), gbc);
        gbc.gridx = 3;
        ...
        for (int i = 0; i < 3; i++) {
            final int index = i; // Fix for effectively final variable
            int row = i * 2 + 2;
            ...
            gbc.gridx = 1;
            loaiCombos[i] = new JComboBox<>(loaiOptions);
            loaiCombos[i].addActionListener(e -> controller.handleLoaiGiayToChange(index));
            formPanel.add(loaiCombos[i], gbc);
            gbc.gridx = 2;
            formPanel.add(new JLabel("Mã:"), gbc);
            gbc.gridx = 3;
    ...
    private static JPanel createButtonPanel(MainFrameView mainFrame, CustomerBookingController controller) {
        JPanel btnPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        btnPanel.setBackground(UIManager.getColor("Panel.background"));
        ...
        btnQuayLai.addActionListener(e -> controller.handleQuayLai());
        btnXacNhan.addActionListener(e -> controller.handleBookingConfirmation());
    ...
    public static JTextField[] getHoTenFields() {
        return hoTenFields;
    }

    public static JComboBox<String>[] getLoaiCombos() {
        return loaiCombos;
    }

    public static JTextField[] getMaFields() {
        return maFields;
    }

    public static JComboBox<Integer> getCbSoNguoi() {
        return cbSoNguoi;
    }

    public static JTextField getTfMaChung() {
        return tfMaChung;
    }

    public static JComboBox<String> getCbLoaiChung() {
        return cbLoaiChung;
    }
}