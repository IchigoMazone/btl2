...
////////                                    .findFirst()
////////                                    .orElse(null);
////////                            // Only add "Yêu cầu hủy" button if the request status is not "Gửi yêu cầu hủy"
////////                            if (request == null || !"Gửi yêu cầu hủy".equalsIgnoreCase(request.getStatus())) {
////////                                JButton btnRequestCancel = new JButton("Yêu cầu hủy");
////////                                styleDialogButton(btnRequestCancel, new Color(255, 165, 0));
////////                                btnRequestCancel.addActionListener(e -> {
////////                                    RequestService.updateStatus(notification.getRequestId(), "Gửi yêu cầu hủy");
...
////                                    .findFirst()
////                                    .orElse(null);
////                            // Only add "Yêu cầu hủy" button if there is no existing cancellation request
...
////            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
////            frame.setSize(700, 500);
////            frame.setContentPane(createNotificationPanel(XML_PATH, "nhat2112"));
////            frame.setLocationRelativeTo(null);
////            frame.setVisible(true);
////        });
////    }
...
//                if (!current.contains("[Đã đọc]")) {
//                    model.setValueAt(current + " [Đã đọc]", row, 0);
//                    NotificationService.updateStatus(notification.getBookingId(), notification.getRequestId(),
//                            notification.getUserName(), notification.getContent(), "Đã đọc");
//                    NotificationXML updatedXML = FileUtils.readFromFile(xmlPath, NotificationXML.class);
//                    if (updatedXML != null) {
//                        notifications.clear();
//                        notifications.addAll(updatedXML.getNotifications().stream()
